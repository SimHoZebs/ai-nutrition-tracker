...
export default function Dashboard() {
  ...
  const [calP, proP, carP, fatP] = useMemo(() => {
    ...
    const todayEntries = historyQuery.data?.filter((entry) => {
    ...
    const carbs = todayEntries.reduce((sum, entry) => sum + entry.carbohydrates, 0);
    const fats = todayEntries.reduce((sum, entry) =>
      sum + entry.trans_fat + entry.saturated_fat + entry.unsaturated_fat, 0);
