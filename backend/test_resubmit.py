...
def test_resubmit_functionality():
    ...
    resubmit_response = session.post(
        f"{API_BASE}/resubmit/", json={"answers": FOLLOW_UP_ANSWERS}, headers=headers
    )
    ...
    if resubmit_response.status_code == 200:
        ...
        try:
            ...
            # Check if response has the "response" field (saved to DB)
            if "response" in final_response_data and final_response_data["response"]:
                ...
                for food in foods:
                    ...
                    if food.get("carbohydrates"):
                        print(
                            f"    Carbs: {food.get('carbohydrates', 0)}g, Fat: {food.get('saturated_fat', 0) + food.get('unsaturated_fat', 0)}g"
                        )
            ...
            # Also check for foods data in the parts structure
            elif "parts" in final_response_data and final_response_data["parts"]:
                for part in final_response_data["parts"]:
                    if "text" in part:
                        try:
                            # Try to parse as JSON array of foods
                            foods_data = json.loads(part["text"])
                            if isinstance(foods_data, list) and len(foods_data) > 0:
                                print(
                                    f"\n🍽️  AI Agent processed {len(foods_data)} food items:"
                                )
                                for food in foods_data:
                                    name = food.get("name", "Unknown")
                                    meal_type = food.get("meal_type", "Unknown meal")
                                    ...
                                    sat_fat = food.get(
                                        "saturated_fat_g", food.get("saturated_fat", 0)
                                    )
                                    unsat_fat = food.get(
                                        "unsaturated_fat_g",
                                        food.get("unsaturated_fat", 0),
                                    )

                                    print(f"  - {name} ({meal_type})")
                                    print(
                                        f"    Calories: {calories}, Protein: {protein}g"
                                    )
                                    print(
                                        f"    Carbs: {carbs}g, Fat: {sat_fat + unsat_fat}g"
                                    )

                                    if food.get("others"):
                                        others = food["others"]
                                        if others.get("sodium_mg"):
                                            print(
                                                f"    Sodium: {others['sodium_mg']}mg"
                                            )
                                        if others.get("cholesterol_mg"):
                                            print(
                                                f"    Cholesterol: {others['cholesterol_mg']}mg"
                                            )

                                print(
                                    f"\n⚠️  Food data received but may not be saved to database yet"
                                )
                                print(
                                    "    (Check Django logs to see if foods were actually saved)"
                                )
                                return True
                        except json.JSONDecodeError:
                            continue

                print("⚠️  Resubmit successful but could not parse food data")
