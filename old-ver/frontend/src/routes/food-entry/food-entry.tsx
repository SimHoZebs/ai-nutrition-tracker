...
export default function FoodEntry() {
  ...
  const foodEntryQuery = useQuery({
    ...
    queryFn: async () => {
      const [resp, status] = await handleRequest(
        "GET",
        `/api/foods/${foodEntryId}`,
      );
  ...
  const nutrientUpdateMutation = useMutation({
    ...
    mutationFn: async ({ nutrient, newValue }: NutrientUpdateData) => {
      const foodEntryNew: { [key: string]: string | number | unknown } = {
        ...foodEntryQuery.data!,
      };
  ...
  return (
    <CenteredPage innerClassName={styles.container}>
      ...
      <div>
        <h2>Nutrients</h2>
        <p style={{ color: "var(--note)" }}>Tap any card to edit</p>
      </div>
      ...
      <div className={styles.nutrientsContainer}>
        <NutrientCard
          ...
          onSave={(newValue: string) =>
            nutrientUpdateMutation.mutate({ nutrient: "calories", newValue })
          }
        />
        <NutrientCard
          nutrient="Protein"
          count={foodEntry.protein}
          unit="g"
          onSave={(newValue: string) =>
            nutrientUpdateMutation.mutate({ nutrient: "protein", newValue })
          }
        />
        <NutrientCard
          nutrient="Carbohydrates"
          count={foodEntry.carbohydrates}
          unit="g"
          onSave={(newValue: string) =>
            nutrientUpdateMutation.mutate({
              nutrient: "carbohydrates",
              newValue,
            })
          }
        ...
        <NutrientCard
          nutrient="Trans Fat"
          count={foodEntry.trans_fat}
          unit="g"
          onSave={(newValue: string) =>
            nutrientUpdateMutation.mutate({ nutrient: "trans_fat", newValue })
          }
        />
        <NutrientCard
          nutrient="Saturated Fat"
          count={foodEntry.saturated_fat}
          unit="g"
          onSave={(newValue: string) =>
            nutrientUpdateMutation.mutate({
        ...
        <NutrientCard
          ...
          count={foodEntry.unsaturated_fat}
          unit="g"
          onSave={(newValue: string) =>
            nutrientUpdateMutation.mutate({
